...
const state = reactive({
  ...
  isLogoStartsWithCorporate: computed(() =>
    CORPORATE_REGEX.test(state.logoText),
  ),
});
</script>
...
.coral {
  background: linear-gradient(
    180deg,
    theme('colors.coral.600') 0%,
    theme('colors.coral.700') 100%
  );
}
...
.peacock {
  background: linear-gradient(
    180deg,
    theme('colors.peacock.600') 0%,
    theme('colors.peacock.700') 100%
  );
}
</style>
