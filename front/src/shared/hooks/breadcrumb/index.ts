...
export const useBreadcrumbs = () => {
  ...
  return {
    breadcrumbs: computed<Breadcrumb[]>(() => {
      ...
      return matched.reduce((results: any[], d) => {
        ...
        if (currentRegexRes.length > 1) {
          ...
          const routeRegexRes =
            matched[matched.length - 1].regex.exec(route.path) ?? [];
          currentRegexRes.forEach((param, i) => {
            if (i !== 0) path = path?.replace(param, routeRegexRes[i]) ?? '';
          });
          location.path = path;
        }

        const copiable =
          typeof d.meta.copiable === 'function'
            ? d.meta.copiable(route)
            : d.meta.copiable;
        if (d.meta.breadcrumbs && typeof d.meta.breadcrumbs === 'function') {
          ...
          if (typeof label === 'function') {
            ...
            if (labelResult)
              results.push({ name: labelResult, to: location, copiable });
          ...
          if (typeof translationId === 'function') {
            ...
            if (Array.isArray(translationIdResult)) {
              ...
              results.push({
                name: i18n.t(id, values),
                to: location,
                copiable,
              });
            } else if (translationIdResult)
              results.push({
