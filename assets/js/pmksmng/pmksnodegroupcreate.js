...
$(document).ready(function() {
    ...
    var connectionName = urlParams.get("connectionName")
    var clusterID = urlParams.get("clusterID")
    var clusterName = urlParams.get("clusterName")

    console.log(connectionName,clusterID,clusterName)
...
function getCommonPmksData(caller, clusterID, connectionName) {
    var url = "/operations/pmksmng/id/" + clusterID + "?connectionName=" + connectionName+"&clusterId="+clusterID
    console.log("get common pMks url : ",url)
    axios.get(url, {
        headers: {
...
function addNodeGroupData(item, nodeGroupIndex, clusterID){
    ...
    //var nodeGroupBadge = '<li class="_sel_cr_ ' + nodeGroupDispClass + '" id="nodeGroupOfCluster_' + nodeGroupIndex + '"><a href="javascript:void(0);" onclick="mcpjs['pmksmng/pmksmng'].clickListOfNodeGroup(\'' + clusterID + '\',\'' + nodeGroupID + '\')"><span class="txt">' + nodeGroupID + '</span></a></li>';
...
// 추가 된 nodeGroup을 저장
export function deployNodeGroup(){
    var clusterID = $("#pmks_cluster_id").val();
    var connectionName = $("#pmks_cluster_connection").val();
    ...
    if( nodeGroupName == ""){
        mcpjs["util/util"].commonAlert("Please Input Name");
        return
    }
    ...
    if( nodeGroupVmSpecId == ""){
        mcpjs["util/util"].commonAlert("Please Input Root Disk Type");
        return
    }
    var nodeGroupRootDiskSize = $("#nodegroup_info_rootdisk_size_" + addNodeGroupIndex).val();
    if( nodeGroupVmSpecId == ""){
        mcpjs["util/util"].commonAlert("Please Input Root Disk Size");
        return
    }
    
    

    var url = "/operations/pmksmng/id/" + clusterID + "/nodegroup"
    console.log("URL : ", url)
    ...
    if (obj) {
        client.post(url, obj, {
            ...
            if (result.status == 200 || result.status == 201) {
                mcpjs["util/util"].commonAlert("NodeGroup Creation Requested")
                // 목록 화면으로 전환
            } else {
                var message = result.data.message;
                mcpjs["util/util"].commonAlert("Failed to Create NodeGroup : " + message + "(" + result.status + ")");
            }
        }).catch((error) => {
            console.warn(error);
