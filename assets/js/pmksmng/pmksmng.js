...
$(document).ready(function () {
    ...
    mcpjs['util/pathfinder'].getCommonAllPmksList("onload");
});
...
function initTable() {
    ...
    var columns = [
        ...
        {title:"Status", field:"status", formatter: statusFormatter, vertAlign: "middle", hozAlign: "center", headerHozAlign: "center", headerSort:false, maxWidth: 100},
        {title:"Id", field:"IId.SystemId", visible: false},
        {title:"Name", field:"IId.NameId"},
        {title: "Provider", field:"ProviderName", headerHozAlign: "center", headerSort:false, maxWidth: 80},
        {title: "VPC", field:"Network.VpcIID.NameId",  headerHozAlign: "center", headerSort:false},
        {title: "Subnet", field:`Network.SubnetIIDs`,  formatter: networkInfoFormatter, formatterParams: {array: "SubnetIIDs"}, headerHozAlign: "center", headerSort:false},
        {title: "Security Group", field:"Network.SecurityGroupIIDs", formatter: networkInfoFormatter, formatterParams: {array: "SecurityGroupIIDs"}, headerHozAlign: "center", headerSort:false},
        {title: "Node Group", field:"", headerHozAlign: "center", headerSort:false},
        
    ]
    
    table = mcpjs["util/util"].setTabulator("pmksList", tableObjParams, columns)
    
    table.on("rowClick", function(e, row){
        clickListOfCluster(row.getCell("IId.NameId").getValue())
    });
...
export function pmksTotalListExport() {
    var namespace = $("#topboxDefaultNameSpaceName").text()
    table.download("csv", namespace.toUpperCase() + "-PMKS.csv", {delimiter:","})
}

function statusFormatter(cell) {
    var mcisStatusCell = '<img title="' + cell.getData().status + '" src="/assets/images/contents/icon_running.png" class="icon" alt="">'
  
    return mcisStatusCell
}

function networkInfoFormatter(cell, param) {
    var data = cell.getData().Network
    var arrayCell = ""
    data[param.array].map(object => {
        arrayCell += object.NameId
    })
...
function displayClusterListArea() {
    $(".dashboard .status_list tbody tr").each(function () {
        var $td_list = $(this),
            $status = $(".detail_box"),// 1단계
            $status_sub = $(".detail_box_sub"),// 2 단계
            $detail = $(".server_info");
...
function displayNodeGroupInfoArea() {
    $(".dashboard .ds_cont .area_cont .listbox li.sel_cr").each(function () {
        ...
        var $status_sub = $(".detail_box_sub")// 2 단계
            console.log(">>>>>");
...
export function getCommonAllPmksListSuccess(caller, data) {
    if (caller == "onload") {

        TOTAL_PMKS_LIST = new Map();
        if (data.length) {
            for (var i in data) {
                var clusterInfo = data[i]
...
function addClusterData(item, index) {
    ...
    html +=
        "<tr onclick=\"mcpjs[\'pmksmng/pmksmng\'].clickListOfCluster('" + clusterID + "', " + index + ");\">"
...
function addNodeGroupData(item, nodeGroupIndex, clusterID) {
    ...
    var nodeGroupBadge = '<li class="sel_cr ' + nodeGroupDispClass + '" id="nodeGroupOfCluster_' + nodeGroupIndex + '"><a href="javascript:void(0);" onclick="mcpjs[\'pmksmng/pmksmng\'].clickListOfNodeGroup(\'' + clusterID + '\',\'' + nodeGroupID + '\')"><span class="txt">' + nodeGroupID + '</span></a></li>';
...
function setNodeList(clusterID, nodeGroupID) {
    ...
    // node 목록
    for (var i in nodeGroupList) {
        var tempNodeGroupInfo = nodeGroupList[i];
        var tempNodeGroupID = tempNodeGroupInfo.IId.NameId;
...
function addNodeRow(item, nodeIndex, clusterID, nodeGroupID) {
    ...
    var nodeBadge = '<li class="sel_cr bgbox_r" id="node_' + nodeIndex + '"><a href="javascript:void(0);" onclick="mcpjs[\'pmksmng/pmksmng\'].clickListOfNode(\'' + clusterID + '\',\'' + nodeGroupID + '\',\'' + nodeID + '\')"><span class="txt">' + nodeName + '</span></a></li>';

    return nodeBadge
}

// Cluster 목록에서 Cluster 클릭 -> Cluster Info + NodeGroup List icon 
export function clickListOfCluster(clusterID, clusterIndex) {
    var clusterInfo = TOTAL_PMKS_LIST.get(clusterID);
    // cluster Info 표시    
    ...
    // 임시 코드
    clusterInfo.NodeGroupList = [{IId: {NameId: "test"}, ImageIID: {NameId: "image"}, VMSpecName: "spec", KeyPairIID: {NameId: "KeyPair"}, DesiredNodeSize: "3", MaxNodeSize: "3", MinNodeSize: "3", OnAutoScaling: false, RootDiskType: "3", RootDiskSize: "3", Nodes: [{NameId: "testvm", SystemId: "testvm"}]}]
    ...
    if (clusterInfo.NodeGroupList != null) {
        ...
        for (var o in clusterInfo.NodeGroupList) {
            ...
            html += '<li id="nodegroup_status_icon_' + o + '" class="sel_cr ' + nodeGroupStatusClass + '"><a href="javascript:void(0);" onclick="mcpjs[\'pmksmng/pmksmng\'].clickListOfNodeGroup(\'' + clusterID + '\',\'' + nodeGroupName + '\')"><span class="txt">' + nodeGroupName + '</span></a></li>';
        }
...
export function clickListOfNodeGroup(clusterID, nodeGrouID) {
    console.log("clickListOfNodeGroup")
    setNodeList(clusterID, nodeGrouID);
}

// Node 목록에서 Node 클릭
export function clickListOfNode(clusterID, nodeGrouID, nodeID) {
...
export function addNewNodeGroup() {
    ...
    // var url = mcpjs['util/pathfinder'].setUrlByParam("/operations/pmksmng/nodegroup/regform", urlParamMap)

    // //onClick="mcpjs['util/util'].changePage('PmksNodeGroupRegForm')"
    // //pmksNodeGroupRegGroup := e.Group("/operations/manages/pmksmng/cluster/:clusterID/regform", pmksNodeGroupRegTemplate)
    // //var url = "/operations/manages/pmksmng/regform/" + clusterId + "/" + clusterName;

    // location.href = url + "?connectionName=" + connectionName+"&clusterID="+clusterID+"&clusterName="+clusterName;

    var targetUrl = "PmksNodeGroupRegForm"
    var optionParamMap = new Map();
    optionParamMap.set("{clusterID}", clusterID)
    optionParamMap.set("connectionName", connectionName)
    optionParamMap.set("clusterID", clusterID)
    optionParamMap.set("clusterName", clusterName)
...
function deleteCluster() {
    ...
    if (checkedCount == 0) {
        mcpjs["util/util"].commonAlert("Please Select a PMKS")
        return;
    } else if (checkedCount > 1) {
        mcpjs["util/util"].commonAlert("Please Select One PMKS at a time")
        return;
    }

    var url = "/operations/manages/pmks/" + clusterID + "?connectionName=" + connectionName;
    client.delete(url, {
        headers: {
        ...
        if (statusCode != 200 && statusCode != 201) {
            ...
            mcpjs["util/util"].commonAlert("PMKS Deletion Requested");
...
function deleteNodeGroupOfPmks() {
    ...
    var url = mcpjs['util/pathfinder'].setUrlByParam("PmksNodeGroupDelProc", urlParamMap)
    console.log("URL : ", url)
    client.delete(url, {
        // headers: {
        //     'Content-Type': "application/json"
        ...
        var statusCode = result.data.status;
        if (statusCode == 200 || statusCode == 201) {
            mcpjs["util/util"].commonAlert("NodeGroup Deleted");

        } else {
            var message = result.data.message;
            mcpjs["util/util"].commonAlert("Fail to Delete NodeGroup : " + message + "(" + statusCode + ")");
...
function UpdateNodeGroupScaleSize(caller) {
    ...
    if (caller == "DesiredNodeSize") {
        if (desiredSize > maxSize) {
            mcpjs["util/util"].commonAlert("too many");
            return
        }
        if (desiredSize < minSize) {
            mcpjs["util/util"].commonAlert("too little");
            return
        }
    ...
    try {
        axios.put(url, new_obj, {
            ...
            if (result.status == 201 || result.status == 200) {
                mcpjs["util/util"].commonResultAlert("Updated")
            } else {
                mcpjs["util/util"].commonAlert("Update Failed")
            }
        }).catch((error) => {
            console.warn(error);
