...
//    });
    ...
    try{
        var url = "/operations/manages/pmksmng/reg/proc"
        client.post(url,new_obj,{
            ...
            if(result.status == 201 || result.status == 200){
                mcpjs["util/util"].commonResultAlert("PMKS create request success")
...
function changeConnectionInfo(connectionName){
    ...
    // vpc : filter
    getVpcInfoListForSelectbox(connectionName, 'regVNetId')
...
function getImageListCallbackFail(caller, error) {
    mcpjs["util/util"].commonAlert("조회 오류 " + error);
}
