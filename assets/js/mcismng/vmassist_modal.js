...
export function showMap() {
	if (JZMap === undefined) { // 지도가 없을 때만 생성, 한 번 생성되면 다시 생성 안 함
		JZMap = mcpjs['ui/mcis.map'].map_init_target("recommend_map")
		mcpjs['ui/mcis.map'].addClickPin(JZMap)
	}
}
...
function getRegionListFilterAtAssist(providerId, targetRegionObj) {
	// region 목록 filter
	selectBoxFilterByText(targetRegionObj, providerId)
...
function filterSpecsByRangeCallbackSuccess(caller, data) {
	...
	if (data.length) {
		...
		mcpjs['util/util'].commonAlert("No result Found")
	}
...
function filterSpecsByRangeCallbackFail() {
	mcpjs['util/util'].commonAlert("Failt to Search Specs")
}
...
function applyAssistSpec() {
	...
	if (selectedSpecID) {
		//<tr onclick="setValueToFormObj('es_imageList', 'tab_vmImage', 'vmImage', '{{$vmImageIndex}}', 'e_imageId');">
		// $("#tab_vmSpecInfo")
...
function setSpecValueToFormObj(selectedId, selectedSpecName, cspSpecName, selectedConnectionName) {
	...
	if (econnectionName != "" && econnectionName != selectedConnectionName) {
		...
		//<input type="hidden" name="vmImage_info" id="vmImage_info_{{$vmImageIndex}}" value="{{$vmImageItem.ID}}|{{$vmImageItem.Name}}|{{$vmImageItem.ConnectionName}}|{{$vmImageItem.CspImageId}}|{{$vmImageItem.CspImageName}}|{{$vmImageItem.GuestOS}}|{{$vmImageItem.Description}}"/>
...
function getConnectionConfigCandidateInfo(index) {
	...
	console.log(specName);
	var url = "/operations/manages/mcismng/mcisdynamiccheck/list"
	var obj = {
		"commonSpec": [specName]
	}
	client.post(url, obj, {
		...
		if (statusCode == 200 || statusCode == 201) {
			...
			mcpjs['util/util'].commonAlert("Get Connection List Failed : " + message + "(" + statusCode + ")");
...
function selectConnectionConfig(connections, selectedCspSpecName, selectedProviderId) {
	...
	connections.forEach(function (candidateConnectionName, connectionIndex) {
			...
			if (candidateConnectionName == vSpecItem.connectionName) {
				...
				if (selectedCspSpecName == vSpecItem.cspSpecName) {
					...
					html += '<tr id="connectionAssist_tr_' + vSpecIndex + '" onclick="setConnectionAndSpec(' + vSpecIndex + ');">'
						+ '     <input type="hidden" id="connectionAssist_provider_' + vSpecIndex + '" value="' + selectedProviderId + '"/>'
		...
		if (!specExist) {
			displayItemsCount++
			var specButton = "<button name='' value='' class='btn_apply btn_co btn_cr_g' onclick=registerSpecOnClick('" + candidateConnectionName + "','" + selectedCspSpecName + "','" + selectedProviderId + "')><span>spec 등록</span></button>"
			html += '<tr>'
	...
	if (displayItemsCount == 0) {
		...
		mcpjs['util/util'].commonAlert("해당 spec을 생성할 수 있는 connection이 없습니다.")
...
function registerSpecOnClick(regCandidateConnection, selectedCspSpecName, regProviderId) {
	$("#t_regProvider").val(regProviderId)
...
function setConnectionAndSpec(index) {
	selectedProviderId = $("#connectionAssist_provider_" + index).val()
	selectedConnection = $("#connectionAssist_connection_" + index).val()
	selectedSpecName = $("#connectionAssist_specName_" + index).val()
	regConnection = $("#ss_connectionName").val()
	console.log("connection: ", regConnection);
	console.log(selctedProviderId);

	$("#t_regProvider").val(selectedProviderId)
...
// selct box option 세팅
function setConnectionsForOptions(connectionList, selctedProviderId) {
	var html = ""
	connectionList.forEach(function (connItem, connIndex) {
		if (selctedProviderId == connItem.ProviderName) {
			html += '<option value="' + connItem.ConfigName + '">' + connItem.ConfigName + '</option>'
		}
	})
	$("#ss_connectionName").empty()
	$("#ss_connectionName").append(html)
}
...
function changeCloudConnection() {
	var selectedProviderId = $("#t_providerId").val()
	var selectedConnection = $("#t_connectionName").val()
	var selectedSpecName = $("#t_spec").val()

	// provider setting
	$("#ss_providerId").val(selectedProviderId)
	...
	// filtering
	setConnectionsForOptions(totalCloudConnectionList, selectedProviderId)
	...
	$("#ss_connectionName").val(selectedConnection)
	$("#ss_spec").val(selectedSpecName)

	$("#t_provider").val("")
	$("#t_connectionName").val("")
	$("#t_spec").val("")
}
...
function checkMcisNameExist() {
	...
	if (mcisName) {
		...
		mcpjs['util/util'].commonPromptOpen("AddNewMcisDynamic")
	}
}