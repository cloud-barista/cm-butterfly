...
export function nodeDone_btn(){
    //var mcksID = $("#mcks_id").val();
    var mcksName = $("#mcks_name").val();
    //var mcksName = $("#current_mcks").val();
    ...
    var workerProviderIdData = new Array();
    var workerRegionNameData = new Array();
    ...
    for(var i=0; i<workerCountLength; i++){   
        ...
        workerProviderIdData.push($("select[name='workerProvider']")[objIndex].value);
        workerRegionNameData.push($("select[name='workerRegion']")[objIndex].value);
    ...
    for(var i=0; i<workerCountData.length; i++){
        ...
        new_worker['providerId'] = workerProviderIdData[i];
        new_worker['regionName'] = workerRegionNameData[i];
    ...
    try{
        ...
        var controllerKeyName = "McksNodeReg";
        var optionParamMap = new Map();
        optionParamMap.set("{mcksName}", mcksName)
        mcpjs["util/pathfinder"].postCommonData('mcksnodecreate',controllerKeyName,optionParamMap, obj, mcpjs['mcksmng/nodeconfigure'].mcksNodeRegCallbackSuccess)
        ...
        //var url = "/operations/manages/mcksmng/" + mcksID + "/" + mcksName + "/reg/proc";
        // var url = "/operations/mcksmng/name/" + mcksName + "/node";
...
// node reg callback
export function mcksNodeRegCallbackSuccess(caller, result){
    console.log("data : ",result);
    console.log("Result Status : ",result.status); 
...
//    console.log("lastIndex=" + lastIndex)
    ...
    addW = addW.replace(/hidden_index/gi, addWorkerIndex);
    addW = addW.replace(/hidden_workerProvider/gi, 'workerProvider_' + addWorkerIndex);
    addW = addW.replace(/hidden_workerRegion/gi, 'workerRegion_' + addWorkerIndex);
...
export function removeWorkerNode(removeWorkerId){
