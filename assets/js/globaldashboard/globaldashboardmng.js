$(function () {
  ...
  //   "dashboard",
  //   "status",
  //   mcpjs["dashboards/dashboardbynamespacemng"].getMcisListCallbackSuccess
  // );
  // JZMap = map_init();
});
var JZMap;
...
function getCloudConnectionListCallbackSuccess(
  caller,
  ...
  if (!isEmpty(connectionConfigList) && connectionConfigList.length) {
    ...
    for (var itemIndex in connectionConfigList) {
      ...
      if (providerConnectionMap.has(aConnectionConfig.ProviderName)) {
        providerConnectionMap.set(
          aConnectionConfig.ProviderName,
          providerConnectionMap.get(aConnectionConfig.ProviderName) + 1
        );
      } else {
  ...
  for (let item of providerConnectionMap) {
    ...
    $("#cpConnectionDetail").append(
      '<li class="bg_etc bg_' +
        item[0].toLowerCase() +
        '"><a href="javascript:void(0);"><span class="conn_cnt">' +
        item[1] +
        '<div class="conn_tit">' +
        item[0] +
        "</div></span></a></li>"
...
function selectMcis(id, name, target, obj) {
  ...
  if ($target.hasClass("active")) {
    ...
    location.href = "/operations/manages/mcismng/mngform?mcisid=" + mcis_id;
    return;
  } else {
