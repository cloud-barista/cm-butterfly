...
function checkDragonFly2(mcis_id, vm_id){
    ...
    $.ajax({
         ...
         error : function(request,status, error){
            ...
            // $("#check_dragonFly").val("400");
            mcpjs["util/util"].commonAlert("It is Not installed Monitoring Agent!!");
...
//vm 의 통계조회
export function getVmMetric(vmChart, chartTarget,target, mcisID, vmID, metric, periodType,statisticsCriteria, duration){
	console.log("====== Start GetMetric ====== ")
	var color = "";
    var metric_size ="";
	...
	var url = "/operations/monitoring/mcisvmmonitoring"    
    console.log("Request URL : ",url)
    client.post(url,{
        headers: { },
        McisID:mcisID,
    ...
    // });
        ...
        try{
           ...
           mcpjs['util/util'].commonErrorAlert(statusCode, errorMessage + " " + metric + " 조회실패") 
        }catch(e){
            var statusCode1 = error.response.status;
            var errorMessage1 = error.response.statusText;
           mcpjs['util/util'].commonErrorAlert(statusCode1, errorMessage1 + " " + metric + " 조회실패") 
        }
    });
